...
def show_poll(request, poll_name):
    ...
    for date in possible_dates:
        ...
        if str(date.date.year) + ':' + str(date.date.month) in rowspan_counts['yearmonths']:
            ...
            rowspan_counts['yearmonths'][str(date.date.year) + ':' + str(date.date.month)] = 1
        if str(date.date.year) + ':' + str(date.date.month) + ':' + str(date.date.day) in rowspan_counts[
            'yearmonthdays']:
            rowspan_counts['yearmonthdays'][
                str(date.date.year) + ':' + str(date.date.month) + ':' + str(date.date.day)] += 1
        else:
            rowspan_counts['yearmonthdays'][
                str(date.date.year) + ':' + str(date.date.month) + ':' + str(date.date.day)] = 1
    dates = []
